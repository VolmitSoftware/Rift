...
public final class TeleportService {
    ...
    public void teleport(Player player, Location destination, Consumer<Boolean> completion) {
        ...
        invokeAsync(player, destination, callback);
    }

    private void invokeAsync(Player player, Location destination, Consumer<Boolean> completion) {
        try {
            ...
            plugin.getLogger().log(Level.WARNING, "Unable to start asynchronous teleport for " + player.getName(), exception);
            completion.accept(false);
        }
    }
}
